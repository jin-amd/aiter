    ...
    def kernel_mxscale_gemm(
            ...
            def _emit_rows(start_wm, a_frags):
                for frag_i in range_constexpr(len(a_frags)):
                    ...
                    for wn_raw in range_constexpr(wmma_n_rep):
                        ...
                        _emit_wmma(
                            accs, wm, wn, a_frags[frag_i], b_frags, a_scales, b_scales
                        )
        ...
        if const_expr(wave_specialized_tdm):
            for i in range_constexpr(pre_loaded):
                dg0 = _pack_dg0(
                    pred_const, active_stage_lds_addr[i], active_addr_lo, active_addr_hi
                )
        ...
        pipeline_fence(
            outstanding=TDM_LOADS_PER_STEP * (num_buffers - 2), use_cluster=use_cluster
        )
